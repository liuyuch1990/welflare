...
public class Generator {
    ...
    public static void generator(String[] args) throws Exception {
        ...
        String module = "test";
        String basePackageName = "com.unicorn.wsp";
        String controllerPackageName = "controller";
        String servicePackageName = "service";
        String repositoryPackageName = "mapper";
        String modelPackageName = "entity";
        ...
        AbstractLoader loader = (AbstractLoader) Mirror.me(TableDescLoader.class).born();
        ...
        for (Map.Entry<String, TableDescriptor> entry : tables.entrySet()) {
            ...
            typeMap.put("entity", modelPackageName);
            typeMap.put("service", servicePackageName);
            typeMap.put("controller", controllerPackageName);
            typeMap.put("mapper", "mapper");
            typeMap.put("serviceImpl", servicePackageName+".impl");
            typeMap.put("VO", modelPackageName+".vo");
            typeMap.put("ETO", modelPackageName+".exportvo");
             types = new String[]{"entity", "mapper", "service","serviceImpl" ,"controller","VO","ETO","mapperxml"};
            for (String type : new String[]{"entity", "mapper", "service","serviceImpl", "controller","VO","ETO","mapperxml"}) {
                ...
                String dir = "D:\\workspace\\电商迭代\\e-commerce-background\\welfare_shopping_platform" + File.separator + outputDir;

                if(type=="entity"){
                    className = Utils.upperCamel(className);
                }else if(type =="VO" || type=="ETO"){
                    className = Utils.upperCamel(className) + type;
                }else if(type=="mapperxml"){
                    dir +="resources/mapper";
                    File file = new File(dir,  "/" + table.getRepositoryClassName() + ".xml");
                ...
                File file = new File(dir+"java", packagePath + "/" + className + ".java");
                log.debug("generate " + file.getName());
                generator.generate(packageName, templatePath, file, force);
